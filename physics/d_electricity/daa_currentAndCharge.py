...
def da_aa_charge_current_time_piax2():
    ...
    q.constant = None
    if option == 1:
        q.questionBase = [f"The current in a certain wire is {current}A.",f"Calculate the charge passing a point in the wire during {statedTime} {minSecs}."]
        q.answerBase = f"{charge}"
        q.answerUnits = 'C'
    if option == 2:
        q.questionBase = [f"{charge} coulombs pass a point in a wire in {statedTime} {minSecs}.",f"What is the average current at this point during this time?"]
        q.answerBase = f"{current}"
        q.answerUnits = 'A'
    if option == 3:
        q.questionBase = [f"{charge} coulombs pass a point in a wire at a current of {current} Amps.",f"How long does this take?"]
        q.answerBase = f"{time}"
        q.answerUnits = 's'
    q.qtype = 'type'
    return q.returnAll()
...
def da_ab_electrons_current_time_piax2():
    ...
    if option == 1:
        q.questionBase = [f"The current in a certain wire is {current}A.",f"Calculate the number of electrons passing a point in the wire during {statedTime} {minSecs}."]
        q.answerBase = f"{charge}"
        q.answerUnits = 'C'
    if option == 2:
        q.questionBase = [f"{charge} electrons pass a point in a wire in {statedTime} {minSecs}.",f"What is the average current at this point during this time?"]
        q.answerBase = f"{current} A"
        q.answerUnits = 'A'
    if option == 3:
        q.questionBase = [f"{charge} electrons pass a point in a wire at a current of {current} Amps.",f"How long does this take?"]
...
def da_ac_electron_beam_experiment_piax2():
    ...
    q.constant = f"e = {eCharge} c"
    if option == 1:
        q.questionBase = [f"In an electron beam experiment, the beam current is {current} A.",f"Calculate the charge which flows along the beam in {statedTime} {minSecs}."]
        q.answerBase = f"{charge}"
        q.answerUnits = 'C'
    if option == 2:
        q.questionBase = [f"In an electron beam experiment, the beam current is {current} A.",f"Calculate the number of electrons which flow along the beam in {statedTime} {minSecs}."]
...
def da_ad_rechargable_battery_piax2():
    ...
    q.questionBase = [f"A certain type of rechargable battery can delivers {current}A for {statedTime} {minSecs} before its voltage drops and it needs to be recharged.",f"Calculate the maximum time it could be used before being recharged if the current drawn from it were {current2} Amps."]
