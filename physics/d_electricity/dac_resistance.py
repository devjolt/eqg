...
def da_ca_resistance_pd_current_piax2():
    ...
    q.constant = None
    if option == 1:
        q.questionBase = [f"The current through a {component} is {current} A when the pd across it is {pd} V.",f"Calculate the resistance at this current."]
    ...
    if option == 2:
        ...
        q.answerUnits = ' V'
    if option == 3:
        q.questionBase = [f"The pd across a {component} is {pd} V when the resistance is {resistance} \u03A9.",f"Calculate the current through this component."]
...
def da_cb_resistivity_uniform_wire_piax2():
    ...
    if option == 1:
        q.questionBase = [f"Calculate the resistance of a uniform {material} cable of a diameter {round(area/3.15159, 2)} and length {length} m.",f"The resistivity of the material is {resistivityConstant} Ohm metres."]
        q.answerBase= f"{resistance}"
        q.answerUnits = ' \u03A9'
    if option == 2:
