...
def db_ba_two_components_parallel_pxax6():
    ...
    q.questionBase = [f"The battery in the circuit above has an emf of {batPd}v and negligible internal resistance.",f"It is connected to a {comp1Pd} v, {comp1P} W {thing} (a) in parallel with another {comp2Pd} v, {comp2P} W {thing} (b).","Calculate:"]
...
def db_bb_two_components_series_pxax4():
    q = ucf.Question(ucf.currentFuncName())
    q.previousQ, q.nextQ, q.currentQname, q.nextQname, q.previousQname = ucf.previousNext(list_callable_functions(), ucf.currentFuncName(), module_path(), ucf.currentFuncName()[0:2], 0, 2)
    batPd, batI, batP, comp1Pd, comp1P, comp1I,comp2Pd, comp2P, comp2I = parallelCircuitSetup()
    thing = componentSetup()
    q.questionBase = [f"A {batPd} v battery of negligible internal resistance as shown above is connected in series with a {comp1Pd} v, {comp1P} W {thing} (a) and a variable resistor (b).",f"When the resistor is adjusted so that the pd. accross the {thing} is {comp2Pd}, work out:"]
...
def db_bc_two_components_series2_pxax4():
    q = ucf.Question(ucf.currentFuncName())
