...
def gx_aa_nuclear_reactor_decay_pxax3():
    ...
    q.questionBase = [f"In an unsual nuclear reactor, some {original_atom[0]} nuclei absorb neutrons to become {absorbing_atom[0]}.",f"These nuclei decay in {extra_neutrons} stages to become nuclei of the isotope {decayed_atom[0]}."]
...
def gx_ac_radioactive_nucleus_alpha_decay_pxax3():
    ...
    q = ucf.Question(name)
    q.previousQ, q.nextQ, q.currentQname, q.nextQname, q.previousQname = ucf.previousNext(list_callable_functions(), ucf.currentFuncName(), module_path(), ucf.currentFuncName()[0:2], 0, 2)
    q.questionBase = ["A radioactive nucleus decays with the emission of an alpha partcile and a gamma-ray photon."]
