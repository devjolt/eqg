...
def e1aaa_minute_hand_of_clock_pxax3():
...
def e1aab_time_pweriod_angle_electric_motor_paxa3():
    q = ucf.Question(ucf.currentFuncName())
...
def e1aac_planet_rotation_pxax5():
    q = ucf.Question(ucf.currentFuncName())
    planet = planetsGen()
    time = randint(1,10)
...
def e1aad_satalite_orbit_pxax5():
    q = ucf.Question(ucf.currentFuncName())
    ...
    angDisDeg = round(angDis*57.3, 7)
    q.questionBase = [f"A {thing} orbits {orbit} km above the surface of {planet[0]} (the diagram above is not accurate in any way), which has a radius of {planet[2]} km.",f"It takes {timeOrbit} minutes per obit.","Calculate:"]
...
def e1abb_object_circular_path_pxax3():
    q = ucf.Question(ucf.currentFuncName())
    time, radius, mass, circumference, speedOfPoint, centAc, centForce, wheel = centAcSetup()
    thing = objectGen()
...
def e1abc_earth_around_sun3_pxax():
    q = ucf.Question(ucf.currentFuncName())
...
def e1abd_hammer_thrower_pxax5():
    q = ucf.Question(ucf.currentFuncName())
    pi = 3.1415826535898
...
def e1acc_racing_track_piax2():
    q = ucf.Question(ucf.currentFuncName())
    mass, gravity, velocity, radius, circumference, centAc, centForce = roadSetup()
    angle = randint(10, 30)
    velocity = round((gravity * radius * math.tan(angle/57.2958))**0.5, 2)
    q.questionBase = [f"At a racing snail circuit, the track is banked at an angle of {angle}° to the horizontal on a bend that has a radius of curvature of {radius} m.",f"Use the equation v² =grtan\u03B8 to calculate the speed of a vehicle on the bend if there is to be no sideways friction."]
...
def e1adb_swing_pxax7():
    ...
    q.questionBase = [f"An innapropriately named swing at {name[randint(0, len(name)-1)]} is {height} m in length.",f"A pleb of mass {mass} kg descends from a position where the swing is horizontal.","Calculate:"]
...
def e1adc_wheel_pxax7():
    ...
    q2 = ["the centripetal acceleration of a person at the perimeter,"]
