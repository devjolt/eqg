...
def e1bab_free_vibrations_pxax3():
    ...
    sub2 = [f"A {randint(1,50)/10} metre {ruler} is clamped to a table so that part of its length projects at right angles from the edge of the table.",f"{q.item(vl.comments)}. A {randint(10, 200)}g {thing} is attached to the free end of the {ruler}. When the free end of the {ruler} is depressed downwards then released, the {thing} oscillates.",f"Describe how you would find out if the oscillations of the {thing} are free oscillations."]
    q.questionPartList = [
...
def e1bbb_acceleration_and_frequency_of_oscillating_object_pxax4():
    ...
    q.questionPartList = [
        {'sub_number': 1, 'sub_question':["the frequency"], 'sub_answer':f'{frequency}Hz', 'sub_mark':1},
...
def e1bca_frequency_and_acceleration_pxax4():
    ...
    q.questionPartList = [
            {'sub_number': 1, 'sub_question':["its frequency"], 'sub_answer':f'{frequency}Hz', 'sub_mark':2},
            {'sub_number': 2, 'sub_question':["its maximum acceleration"], 'sub_answer':f'{maximum_acceleration}m/s\u00b2', 'sub_mark':2},
            ]    
    q.piclink = q.item(vl.pendulum_links)
    q.previousQ, q.nextQ, q.currentQname, q.nextQname, q.previousQname = ucf.previousNext(list_callable_functions(), ucf.currentFuncName(), module_path(), ucf.currentFuncName()[0:2], 0, 2)
    return q.returnAll()
