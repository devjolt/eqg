...
def ba_ab_multiplying_letters12():
    ...
    q = cf.Question(cf.currentFuncName())
    q.questionBase = [f"Simplify this expression:",f"{firstString}"]
...
def ba_ac_collecting_like_terms13():
    ...
    q = cf.Question(cf.currentFuncName())
    q.questionBase = [f"Simplify the following expression:",f"{firstString}"]
    q.answerBase = finalString + str(numberCounter)
    q.previousQ, q.nextQ, q.currentQname, q.nextQname, q.previousQname = cf.previousNext(list_callable_functions(), cf.currentFuncName(), module_path(), cf.currentFuncName()[0:2], 0, 2)
    return q.returnAll()

def bb_aa_brackets_three_letters13():
    letter1, letter2, letter3 = letterGen(), letterGen(), letterGen()
    ops = ["-", "+"]
    op = ops[randint(0, len(ops)-1)]
    q = cf.Question(cf.currentFuncName())
    q.questionBase = [f"Expand the brackets:",f"{letter1}({letter2} {op} {letter3})"]
    q.answerBase = f"{letter1}{letter2} {op} {letter1}{letter3}"
    q.previousQ, q.nextQ, q.currentQname, q.nextQname, q.previousQname = cf.previousNext(list_callable_functions(), cf.currentFuncName(), module_path(), cf.currentFuncName()[0:2], 0, 2)
    return q.returnAll()

def bb_ab_brackets_numbers_letters13():
...
def bb_ad_factorising_two_letter_terms25():
    ...
    questionBase = [f"Factorise:",f"{(finalTermGen(power(t12)))} {op} {(finalTermGen(power(t13)))}"]
    q = cf.Question(cf.currentFuncName())
    q.questionBase, q.answerBase = questionBase, answer
    q.previousQ, q.nextQ, q.currentQname, q.nextQname, q.previousQname = cf.previousNext(list_callable_functions(), cf.currentFuncName(), module_path(), cf.currentFuncName()[0:2], 0, 2)
    return q.returnAll()
    
def bb_ae_factorising_more_letters25():
...
def bc_ac_one_to_the_power_of15():
    value = 1
    power = randint(1,99)
    SUP = str.maketrans("0123456789", "⁰¹²³⁴⁵⁶⁷⁸⁹")
...
def bc_ad_multiplying_powers15():
...
def bc_af_raising_to_a_power15():
    letterNumber = randint(0,1)
    if letterNumber == 0: value = letterGen()
    else: value = randint(1, 99)
...
def bc_ag_raising_two_powers_to15():
    letterNumber = randint(0,1)
    if letterNumber == 0: value = letterGen()
    else: value = randint(1, 99)
...
def bd_aa_power_with_fractions15():
    ...
    powerSUP = f"{power}".translate(SUP)
    q = cf.Question(cf.currentFuncName())
    q.questionBase = [f"Evaluate:",f"({number1}\u2044{number2}){powerSUP}"]    
    q.answerBase = f"{number1**power}\u2044{number2**power}"
    q.previousQ, q.nextQ, q.currentQname, q.nextQname, q.previousQname = cf.previousNext(list_callable_functions(), cf.currentFuncName(), module_path(), cf.currentFuncName()[0:2], 0, 2)
    return q.returnAll()
    

def bd_ab_power_with_fractions_letters13():
...
def bd_ac_negative_powers17():
    number1 = randint(1,9)
    power = randint(1, 4)
    SUP = str.maketrans("0123456789", "⁰¹²³⁴⁵⁶⁷⁸⁹")
    powerSUP = f"{power}".translate(SUP)
    q = cf.Question(cf.currentFuncName())
    q.questionBase = [f"Evaluate:",f"{number1}\u207b{powerSUP}"]
    q.answerBase = f"1\u2044{number1**power}"
    q.previousQ, q.nextQ, q.currentQname, q.nextQname, q.previousQname = cf.previousNext(list_callable_functions(), cf.currentFuncName(), module_path(), cf.currentFuncName()[0:2], 0, 2)
    return q.returnAll()
    
def bd_ad_negative_powers_with_letter_terms17():
    number1 = letterGen()
    power = randint(1, 9)
    SUP = str.maketrans("0123456789", "⁰¹²³⁴⁵⁶⁷⁸⁹")
    powerSUP = f"{power}".translate(SUP)
...
def bd_ae_fractional_powers17():
    ...
    powerSUP = f"{power}".translate(SUP)
    q = cf.Question(cf.currentFuncName())
    q.questionBase = ["Evaluate:",f"{number1} \u00b9\u2e0d{powerSUP}"]
    q.answerBase = f"{ans}"
    q.previousQ, q.nextQ, q.currentQname, q.nextQname, q.previousQname = cf.previousNext(list_callable_functions(), cf.currentFuncName(), module_path(), cf.currentFuncName()[0:2], 0, 2)
    return q.returnAll()
    
def be_aa_surd_rule1_17():
...
def be_ai_surd_rule7a_17():
    ...
    \u2500\u2500\u2500
     {char2}'''
    q = cf.Question(cf.currentFuncName())
    q.questionBase = "Simplify:"
    q.codeBase = pre if pre else None
    q.answerCodeBase = preans if preans else None
    q.previousQ, q.nextQ, q.currentQname, q.nextQname, q.previousQname = cf.previousNext(list_callable_functions(), cf.currentFuncName(), module_path(), cf.currentFuncName()[0:2], 0, 2)
    return q.returnAll()
    
def be_aj_surd_rule7b_17():
    ...
    q.codeBase = pre if pre else None
    q.answerCodeBase = preans if preans else None
    q.questionBase, q.answerBase = "Simplify:", answer
    q.previousQ, q.nextQ, q.currentQname, q.nextQname, q.previousQname = cf.previousNext(list_callable_functions(), cf.currentFuncName(), module_path(), cf.currentFuncName()[0:2], 0, 2)
    return q.returnAll()
    
def be_ak_surd_simplify_addition_a_38():
    ...
    questionBase = [
        f"Write",f"\u221A{qu1} + \u221A{qu2} + \u221A{qu3}",
        f"in the form {letter}\u221A{surd} where {letter} is an integer."]
...
def be_al_surd_simplify_addition_b_38():
    surd, letter = randint(2, 9), letterGen()
    a1, a2, a3, a4 = randint(1,12),randint(1,12),randint(1,12),randint(1,12) 
    qu1, qu2, qu3, qu4 = (a1**2)*surd, (a2**2)*surd,(a3**2)*surd,(a4**2)*surd
    questionBase = [
...
def be_am_surd_simplify_additionc_38():
    surd, letter = randint(2, 9), letterGen()
    a1, a2, a3, a4 = randint(1,12),randint(1,12),randint(1,12),randint(1,12) 
    qu1, qu2, qu3, qu4 = (a1**2)*surd, (a2**2)*surd,(a3**2)*surd,(a4**2)*surd
    questionBase = [
...
def be_an_surd_simplify_area_38():
    ...
    area = squared * known
    questionBase = [
        f"A rectangle with length {known}{letter} cm and width {letter} cm has an area of {area} cm\u00b2.",
        f"Find the exact value of {letter} giving your answer in its simplest form."]
...
def bf_ab_collect_values_to_solve25():
    x = randint(2,12)
    answer = (f"x = {x}")
    newx = randint(2, 5)
    newint = newx * x
...
def bf_ac_collect_terms_and_divide_to_solve25():
    x = randint(2,12)
    answer = (f"x = {x}")
    newx = randint(2, 5)
    newint = newx * x
...
def bf_ad_expand_brackets_collect_terms_and_divide_to_solve25():
    x = randint(2,6)
    answer = (f"x = {x}")
    print(answer)
    newx = randint(2, 5)
...
def bf_ae_expand_double_brackets_collect_terms_and_divide_to_solve35():
    x = randint(2,12)
    answer = (f"x = {x}")
    print(answer)
    newx = randint(2, 5)
...
def bf_af_solving_with_negative_values35():
    multnum, addx = 0, 0
...
def bf_ae_solving_with_negative_values_2_35():
    x = 0
    while x == 0:
        x = randint(-12, 12)
    xmultnum, addx = 0, 0
...
def bf_af_solve_posneg_collectx_collectint_divide_brackets_fractions35():
    x = 0
    while x == 0:
        x = randint(-12, 12)
    xmultnum, addx = 0, 0
...
def bf_ah_solve_posneg_collectx_collectint_divide_brackets_first_fraction33():
    x = 0
    while x == 0:
        x = randint(-12, 12)
    xmultnum, addx = 0, 0
...
def bf_ai_solve_posneg_collectx_collectint_divide_brackets_second_fraction35():
    x = 0
    while x == 0:
        x = randint(-12, 12)
    xmultnum, addx = 0, 0
